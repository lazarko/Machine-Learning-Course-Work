...
class RandForest(object):
    ...
    def trainClassifier(self, Xtr, yTr, W=None):
        ...
        rfc = RandForest()
        rfc.classifier = RandomForestClassifier(n_estimators = 100,
                                                random_state =42,
                                                max_features = "log2")


        if W is None:
            rfc.classifier.fit(Xtr, yTr)
        else:
            rfc.classifier.fit(Xtr, yTr, sample_weight=W.flatten())
        rfc.trained = True
        return rfc
...
class KnnClass(object):
    ...
    def trainClassifier(self, Xtr, yTr, W=None):
        ...
        rfc = KnnClass()
        if W is None:
            rfc.classifier = KNeighborsClassifier(n_neighbors=9)
        else:
            rfc.classifier = KNeighborsClassifier(n_neighbors=9)
...
class SVM(object):
    ...
    def trainClassifier(self, Xtr, yTr, W=None):
        ...
        svc.classifier = SVC(kernel='poly', degree=9)
        svc.classifier.fit(Xtr, yTr)
